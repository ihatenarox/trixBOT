    ...
    if (message.member.hasPermission('ADMINISTRATOR')) {
        if (message.mentions.channels.array()[0]) {
            ...
            db_config.findOne({ guildid: message.guild.id }, (err, guild) => {
                if (guild) {
                    guild.saida_channel = canal.id
                    guild.save()

                    const embed_final = new Discord.RichEmbed()
                        .setColor('#4682B4')
                        .setDescription(`**${message.member}, Canal ${canal} setado com sucesso para as mensagens de saida!**`)
            ...
            const embed_err = new Discord.RichEmbed()
                .setColor('#ff0000')
                .setDescription(`${message.member}**, Mencione o canal que você deseja que seja o canal de mensagens de saida!**`)
